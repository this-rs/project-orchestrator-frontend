...
export function MilestoneDetailPage() {
  ...
  const sectionIds = ['progress', 'tasks', 'plans', 'projects']
          ...
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" onClick={() => linkDialog.open({
              ...
              onLink: async (taskId) => {
                ...
                // Re-fetch tasks from plans and refresh progress
          ...
          {milestoneTasks.length === 0 ? (
            ...
            <div className="space-y-2">
              {milestoneTasks.map((task) => (
                <div key={task.id} className="flex items-center justify-between p-3 bg-white/[0.06] rounded-lg">
                  <Link
                    to={`/tasks/${task.id}`}
                    className="font-medium text-gray-200 hover:text-indigo-400 transition-colors flex-1 min-w-0 truncate"
                  >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={async () => {
                        await workspacesApi.removeTaskFromMilestone(milestoneId!, task.id)
                        setMilestoneTasks(prev => prev.filter(t => t.id !== task.id))
                        const newProgress = await workspacesApi.getMilestoneProgress(milestoneId!).catch(() => null)
                        setProgress(newProgress)
